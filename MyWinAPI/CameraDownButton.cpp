...
CameraDownButton::CameraDownButton()
	:m_texture(nullptr), m_isOnWork(false), m_time(0)
{
	m_texture = ResourceMgr::GetInstance()->loadTexture(L"CameraDownButton", L"texture\\switch_button_down.png");
...
void CameraDownButton::render(HDC _dc, Graphics* _graphic)
{
	int width = m_texture->getWidth();
	int height = m_texture->getHeight();
...
void CameraDownButton::onMouseClicked()
{
	SoundMgr::GetInstance()->stopBGM();
	SoundMgr::GetInstance()->setBlueUpSide(false);
